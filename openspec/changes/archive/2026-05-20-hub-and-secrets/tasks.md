...
- [x] 1.1 Create `src/ase_cli/checkers/agents_hub_structure.py` — `AgentsHubStructure` class, checks `.agents/`, `.agents/instructions/`, `.agents/skills/` exist, reports missing items, registered via `@registry.register`
...
- [x] 2.1 Create `src/ase_cli/checkers/secrets.py` — `Secrets` class, scans text files for three patterns (private key, AWS key, credential assignment), skips `.git/node_modules/.venv/__pycache__`
...
- [x] 3.1 Update `src/ase_cli/checkers/__init__.py` to import `agents_hub_structure`, `secrets`
