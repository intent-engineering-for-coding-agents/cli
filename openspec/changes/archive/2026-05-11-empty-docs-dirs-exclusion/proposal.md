...
- New shared helper `src/iec_cli/checkers/_shared.py` with `is_effectively_empty(dirpath: Path) -> bool`. Returns `True` when the directory's recursive subtree contains no regular files except entries whose name begins with `.` (e.g. `.gitkeep`, `.hidden`, `.DS_Store`).
...
- **New module**: `src/iec_cli/checkers/_shared.py`
- **Modified modules**: `src/iec_cli/checkers/docs_readme_exists.py`, `docs_index_exists.py`
...
- **Downstream**: The intent-book repo's `docs/design/` placeholder stops failing CI.
- **No new dependencies**.
