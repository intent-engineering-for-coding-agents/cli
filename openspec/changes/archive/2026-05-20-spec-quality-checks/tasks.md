...
- [x] 1.1 Add `find_spec_files(path)` to `src/ase_cli/checkers/_shared.py` — scans `openspec/specs/**/*.md` and active change specs, excluding `archive/`

## 2. spec-ac-ids — implementation and proof

- [x] 2.1 Create `src/ase_cli/checkers/spec_ac_ids.py` — `SpecAcIds` class, uses `find_spec_files`, checks every `Scenario:` heading for `[PREFIX-NNN]` AC ID, registered via `@registry.register`
...
- [x] 3.1 Create `src/ase_cli/checkers/spec_test_category.py` — `SpecTestCategory` class, splits content on scenario headings, checks each body for `**Test:**`, registered via `@registry.register`
...
- [x] 4.1 Update `src/ase_cli/checkers/__init__.py` to import `spec_ac_ids`, `spec_test_category`
