...
- [x] 1.1 Create `src/iec_cli/checkers/spec_size.py` — `SpecSize` class, uses `find_spec_files`, counts lines per file, returns WARN if over configurable limit (env `ASE_SPEC_MAX_LINES`, default 500)
...
- [x] 2.1 Create `src/iec_cli/checkers/file_size.py` — `FileSize` class, walks `*.md` skipping `.git/node_modules/.venv/.vitepress/__pycache__`, counts lines, returns WARN if over limit (env `ASE_FILE_MAX_LINES`, default 500)
...
- [x] 3.1 Update `src/iec_cli/checkers/__init__.py` to import `spec_size`, `file_size`
