...
`docs/decisions/` in iec-cli holds 6 MADR-format ADRs. The MADR convention requires a 4-digit numeric prefix in the filename and specific section headings. Currently nothing enforces this, so a malformed ADR would silently pass validation.
