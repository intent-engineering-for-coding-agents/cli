...
Adopting Intent Engineering practices currently requires manually creating the canonical directory structure — `docs/`, `openspec/`, `.agents/`, `AGENTS.md` — and remembering every subdirectory, every README stub, every convention. That's tedious, error-prone, and a barrier to adoption. `iec init` automates the bootstrap so any repo can adopt Intent Engineering conventions in a single command.

## What Changes

- **New command `iec init`** — scaffolds the canonical Intent Engineering directory structure in the current repo (or a specified path). Creates empty directories, stub files (AGENTS.md, README.md, INDEX.md, testing-convention.md, testing-strategy.md stub), and an AGENTS.md skeleton.
...
- **Idempotent** — running `iec init` on an already-initialized repo is safe; existing files are never overwritten unless `--force` is passed.

## Capabilities

### New Capabilities

- `scaffold-command`: The `iec init` subcommand that creates the canonical Intent Engineering directory structure (`docs/`, `openspec/`, `.agents/`, `AGENTS.md` skeleton) in a target repository. Supports dry-run, force overwrite, and path targeting.
...
- New CLI entry point: `iec init` subcommand added to `iec_cli.main` via Typer
- New source module: `src/iec_cli/init.py` (or `src/iec_cli/commands/init.py`)
- Directory scaffolding utilities: creating dirs, writing stub files
- `docs/testing-convention.md` ships as full static content (generic Intent Engineering testing conventions)
