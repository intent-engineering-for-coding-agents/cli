...
- [x] 1.1 Create `src/ase_cli/checkers/` package with `__init__.py` that imports all checker modules

## 2. agents-exists — implementation and proof

- [x] 2.1 Create `src/ase_cli/checkers/agents_exists.py` with `AgentsExists` checker class, decorated with `@registry.register`
...
- [x] 3.1 Create `src/ase_cli/checkers/agents_size.py` with `AgentsSize` checker class, reading `ASE_AGENTS_MAX_LINES` env var (default 50)
...
- [x] 4.1 Create `src/ase_cli/checkers/agents_links.py` with `AgentsLinks` checker class, parsing `[text](url)` links and detecting bare links
...
- [x] 5.1 Implement `_load_checkers()` in `src/ase_cli/check.py` to import `ase_cli.checkers` package
- [x] 5.2 Integration test: `ase check` runs all three checkers against ase-cli's own repo [AGEX-001, AGSZ-001, AGLN-001]
- [x] 5.3 Integration test: `ase check` reports FAIL when AGENTS.md is missing in an empty directory [AGEX-002, AGSZ-004, AGLN-005]
